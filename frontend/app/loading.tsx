...
export default function Loading() {
    ...
    return (
        <AnimatePresenceProvider>
            {loading && (
                <motion.div initial={{ opacity: 1 }} animate={{ opacity: 1, scale: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="bg-background fixed top-0 left-0 z-50 h-screen w-screen flex justify-center items-center">
                    <div className="flex justify-center items-center flex-col h-full w-full"> 
                        <Image unoptimized src={theme === "dark" ? Logo_Animation : Logo_Animation_Dark} alt="TheVelox" className="w-[50%] md:w-[30%] xl:w-[400px] object-contain xl:h-[200px]" />
                        <Progress value={progress} className=" w-[50%] xl:w-full max-w-[500px] h-[2px]" />
                    </div>
