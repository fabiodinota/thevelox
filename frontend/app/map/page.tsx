...
const page = () => {
    ...
    const updateSearchReqData = (data: ISearchReqData | {}) => {
        ...
        if (Object.keys(data).length === 0 || ['startStation', 'endStation', 'lines', 'path'].every(key => key in data)) {
    ...
    useEffect(() => {
        const searchData = JSON.parse(localStorage.getItem("searchReqData") || "{}");
    ...
    const handleSearch = async () => {
        ...
        await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/map/search?startStation=${startStation}&endStation=${endStation}`, { withCredentials: true })
            .then((res) => {
                console.log("Res: ", res);
    ...
    useEffect(() => {
        if (searching) {
            localStorage.setItem("searchReqData", JSON.stringify(searchReqData));
        }
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative">
        <div style={{ backdropFilter: "blur(16px)"}} className="absolute left-1/2 -translate-x-1/2 bottom-10 z-[9999] backdrop-bl bg-white/20 flex justify-center items-center border border-gray-100 w-full max-w-[900px] h-[80px] rounded-xl">
            ...
            />
            {/* search button */}
            <button onClick={searching ? handleGoBackToMap : handleSearch} className="w-full h-[50px] bg-blue-500 text-white rounded-xl">
            {searching ? (
